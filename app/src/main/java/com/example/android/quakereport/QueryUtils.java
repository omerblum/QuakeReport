...
public final class QueryUtils {
    ...
    public static ArrayList<EarthQuake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray featArray = root.optJSONArray("features");
            for (int i=0; i<featArray.length(); i++){
                JSONObject tempJson = featArray.getJSONObject(i);
                double mag = tempJson.getJSONObject("properties").getDouble("mag");
                String loc = tempJson.getJSONObject("properties").optString("place");
                long time = tempJson.getJSONObject("properties").getLong("time");
                String url = tempJson.getJSONObject("properties").getString("url");
                EarthQuake tempEarth = new EarthQuake(mag, loc, time, url);
                earthquakes.add(tempEarth);
            }
